 ...
 */
export const functionify = <T = DefaultTheme>(target: T, path: Array<string> = []): Functionified<T> => {
  return Object.keys(target).reduce<Record<any, any>>((p, key) => {
    ...
    } else if (['string', 'number', 'boolean'].includes(typeof target[key as keyof T])) {
      p[key] = (props: ThemeProps<DefaultTheme>) => traverse(props.theme, fullPath)
    } else {
